chiral-infrastructure-as-code: generate mirrored, native cloud templates from a single source of truth. aws and azure in sync, without third-party vendor lock-in.

---

Elevator Pitch: We use the Chiral Pattern to avoid IaC (Infrastructure as Code) lock-in. Instead of relying on 3rd-party state managers, we maintain a central Nucleus (Intent) and use the Chiral Engine to synthesize first-party Enantiomers (Native Templates) for both AWS and Azure. This ensures we have native support and zero vendor dependency.
