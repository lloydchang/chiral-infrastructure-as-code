...
// =================================================================
const skillsBicep = AzureBicepAdapter.generateSkillsBicep(intent, azureRegion);
bicepContent += skillsBicep;
...
output adfsVmId string = adfsVm.id
  ...
  private static generateSkillsBicep(intent: ChiralSystem, azureRegion: string): string {
    if (!intent.skills) return '';

    let skillsBicep = '\n\n// =================================================================\n';
    skillsBicep += '// 5. AI AGENT SKILLS (Optional - Level 3 Constructs)\n';
    skillsBicep += '// =================================================================\n';

    // Storage Account for skills that need blob storage
    if (intent.skills.imageProcessing || intent.skills.automation) {
      skillsBicep += `
// Storage Account for AI Skills
resource skillsStorage 'Microsoft.Storage/storageAccounts@2023-01-01' = {
  name: '${intent.projectName}skillstorage'
...
`;
    ...
    // Image Processing Skill - Azure Functions
    if (intent.skills.imageProcessing) {
      const memorySize = intent.skills.imageProcessing.performance === 'high' ? 2048 :
                        intent.skills.imageProcessing.performance === 'medium' ? 1024 : 512;

      skillsBicep += `
// Image Processing Function App
resource imageProcessingApp 'Microsoft.Web/sites@2023-01-01' = {
  name: '${intent.projectName}-image-processing'
  location: '${azureRegion}'
  kind: 'functionapp'
  properties: {
    serverFarmId: '${intent.projectName}-plan'
    siteConfig: {
      linuxFxVersion: 'PYTHON|3.9'
      appSettings: [
        {
          name: 'AzureWebJobsStorage'
          value: 'DefaultEndpointsProtocol=https;AccountName=${intent.projectName}skillstorage;AccountKey=;EndpointSuffix=core.windows.net'
        }
        {
        ...
        {
          name: 'CAPABILITY'
          value: '${intent.skills.imageProcessing.capability}'
        }
        {
          name: 'PERFORMANCE'
          value: '${intent.skills.imageProcessing.performance}'
        }
      ]
...
`;
    ...
    // Data Analysis Skill - Azure Functions
    if (intent.skills.dataAnalysis) {
      const runtime = intent.skills.dataAnalysis.framework === 'tensorflow' ? 'python' :
                     intent.skills.dataAnalysis.framework === 'pytorch' ? 'python' : 'python';

      skillsBicep += `
// Data Analysis Function App
resource dataAnalysisApp 'Microsoft.Web/sites@2023-01-01' = {
  ...
  properties: {
    serverFarmId: '${intent.projectName}-plan'
    siteConfig: {
      linuxFxVersion: '${runtime.toUpperCase()}|3.9'
      appSettings: [
        {
          name: 'AzureWebJobsStorage'
          value: 'DefaultEndpointsProtocol=https;AccountName=${intent.projectName}skillstorage;AccountKey=;EndpointSuffix=core.windows.net'
        ...
        {
          name: 'FRAMEWORK'
          value: '${intent.skills.dataAnalysis.framework}'
        }
        {
          name: 'CAPABILITY'
          value: '${intent.skills.dataAnalysis.capability}'
        }
      ]
...
`;
    ...
    // Natural Language Skill - Azure Functions
    if (intent.skills.naturalLanguage) {
      const memorySize = intent.skills.naturalLanguage.modelSize === 'large' ? 4096 :
                        intent.skills.naturalLanguage.modelSize === 'medium' ? 2048 : 1024;

      skillsBicep += `
// Natural Language Function App
resource naturalLanguageApp 'Microsoft.Web/sites@2023-01-01' = {
  ...
  properties: {
    ...
    siteConfig: {
      ...
      appSettings: [
        ...
        {
          name: 'CAPABILITY'
          value: '${intent.skills.naturalLanguage.capability}'
        }
        {
          name: 'MODEL_SIZE'
          value: '${intent.skills.naturalLanguage.modelSize}'
        }
      ]
...
`;
    ...
    // Automation Skill - Azure Functions
    if (intent.skills.automation) {
      const timeout = intent.skills.automation.complexity === 'complex' ? '00:15:00' :
                     intent.skills.automation.complexity === 'moderate' ? '00:10:00' : '00:05:00';

      skillsBicep += `
// Automation Function App
resource automationApp 'Microsoft.Web/sites@2023-01-01' = {
  ...
  properties: {
    ...
    siteConfig: {
      ...
      appSettings: [
        ...
        {
          name: 'CAPABILITY'
          value: '${intent.skills.automation.capability}'
        }
        {
          name: 'COMPLEXITY'
          value: '${intent.skills.automation.complexity}'
        }
      ]
      functionAppScaleLimit: 100
    }
