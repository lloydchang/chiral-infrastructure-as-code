  ...
  private async assessGDPR(config: ChiralSystem): Promise<SecurityViolation[]> {
    ...
    if (!config.compliance?.consentManagement) {
      violations.push({
        ...
        control: 'Consent Management',
        title: 'Consent management missing',
        description: 'GDPR requires explicit consent management procedures',
        impact: 'Non-compliance with GDPR Article 7',
        affectedResources: ['all'],
        remediation: 'Implement explicit consent management procedures'
      });
    }
    ...
    if (!config.compliance?.dataSubjectRights) {
      violations.push({
        ...
        control: 'Data Subject Rights',
        title: 'Data subject rights support missing',
        description: 'GDPR requires support for data subject rights (access, rectification, erasure)',
        impact: 'Non-compliance with GDPR Articles 15-17',
        affectedResources: ['all'],
        remediation: 'Implement procedures for data subject rights (access, rectification, erasure)'
      });
    }
    ...
    if (!config.compliance?.breachNotification) {
      violations.push({
        ...
        category: 'privacy',
        control: 'Breach Notification',
        title: '72-hour breach notification missing',
        description: 'GDPR requires 72-hour breach notification to supervisory authorities',
    ...
    if (!config.compliance?.privacyImpactAssessment) {
      violations.push({
        ...
        title: 'Data protection impact assessment missing',
    ...
    if (!config.compliance?.dataMinimization) {
      violations.push({
        ...
        category: 'privacy',
        control: 'Data Minimization',
        title: 'Data minimization principle missing',
        description: 'GDPR requires data minimization principles',
    ...
    if (!config.compliance?.retentionPolicy?.piiRetentionDays) {
      violations.push({
        ...
        severity: 'high',
        category: 'privacy',
        control: 'Storage Limitation',
        title: 'Storage limitation principle missing',
        description: 'GDPR requires storage limitation principles',
        impact: 'Non-compliance with GDPR Article 5(1)(e)',
        affectedResources: ['database', 'storage'],
